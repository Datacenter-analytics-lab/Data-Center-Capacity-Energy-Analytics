...
The headline result: the site's original cooling design (2 chillers of 450 kWth) looked comfortably sized on paper but **did not actually hold N+1 redundancy** at peak load. Re-splitting the same installed capacity into 3 chillers of 300 kWth restored it **at the same installed capacity**, with no increase in kWth. That correction, and its knock-on effects on PUE, growth capacity, and decision-making, is the throughline of everything below.

> **North Star Metric: Binding Constraint N+1 Headroom.** The lowest N+1 margin among the three critical subsystems (electrical, UPS, cooling). Whichever one it is, that's the number that actually caps growth and defines "safe." It currently sits at cooling (+74 kWth), but the metric isn't tied to cooling specifically: if cooling capacity were increased further, this North Star would simply track whichever subsystem became the new binding constraint.
>
...
*Average and peak values are each the mean and the highest observed independently over the 28-day simulation. The facility power peak and the IT load peak occur at different moments (see §8 and the Peak Energy Balance table in §7), driven by a lower cooling COP at that specific interval, not by higher IT demand.*
...
Strict star schema. Each fact table represents an independent measurement or simulated telemetry stream. Derived engineering quantities (like `FACT_Thermal_Load`, built from IT load plus losses, lighting and occupants) are explicitly identified as model outputs, not raw sensor readings. No fact-to-fact relationships: everything correlates through shared dimensions.
...
| Room\_ID     | text     | Room A / Room B |
| IT\_Load\_kW | decimal  | IT power draw   |
...
| ---------------- | -------- | ------------------------------------- |
| Timestamp        | datetime | 15-min interval                       |
| UPS\_ID          | text     | Equipment identifier                  |
| Output\_Load\_kW | decimal  | Power delivered to IT                 |
| UPS\_Loss\_kW    | decimal  | No-load + proportional loss           |
| Load\_Percent    | decimal  | Output ÷ rated capacity               |
...
| ---------------------------------------- | -------- | -------------------------- |
| Timestamp                                | datetime | 15-min interval            |
| IT\_Thermal\_Load\_kWth                  | decimal  | IT heat (1:1 from IT load) |
| UPS\_Thermal\_Load\_kWth                 | decimal  | UPS loss as heat           |
| Power\_Distribution\_Thermal\_Load\_kWth | decimal  | Distribution loss as heat  |
| Total\_Thermal\_Load\_kWth               | decimal  | Sum of all thermal sources |
...
| ---------------------- | -------- | ------------------------- |
| Timestamp              | datetime | 15-min interval           |
| Cooling\_Equipment\_ID | text     | Chiller identifier        |
| Cooling\_Power\_kW     | decimal  | Electrical input          |
| Cooling\_Output\_kWth  | decimal  | Thermal output            |
| Efficiency\_COP        | decimal  | Output ÷ electrical input |
| Status                 | text     | Active / Degraded         |
| Alarm\_Code            | text     | Linked alarm, if any      |
...
| ----------- | -------- | ----------------------- |
| Timestamp   | datetime | 15-min interval         |
| Power\_kW   | decimal  | Total facility power    |
...
| Column                            | Type     | Meaning                                                   |
| --------------------------------- | -------- | --------------------------------------------------------- |
| Alarm\_ID                         | text     | Event identifier (1 deliberate duplicate)                 |
| Start\_Timestamp / End\_Timestamp | datetime | Event window                                              |
| Equipment\_ID                     | text     | Linked equipment                                          |
| Service\_Impacting                | boolean  | Component-level impact flag                               |
| IT\_Outage                        | boolean  | Actual IT service interruption (always False in this run) |

## Assumptions & planning parameters

`DIM_Scenario` is the planning layer used for capacity projections in §9:
...
| ------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Simulation period         | 28 days, 15-min intervals | Model design. Fit for capacity and trend analysis, coarser than a real-time monitoring (DCIM/BMS) feed, which typically samples every 1 to 5 minutes for incident detection |
...
Data generation uses no non-deterministic random function: noise is a trigonometric hash indexed on timestamp and equipment ID, so every model refresh produces identical results. That is a reproducibility requirement, not a cosmetic detail.
...
Raising the cooling supply setpoint from 18°C to 22°C would bring PUE to 1.46. That is a **simulated annualized saving under the model's COP sensitivity assumption (+3% per °C)**, not a measured result, and only worth pursuing *after* the capacity fix in §8, since a higher setpoint eats into the thermal margin needed during a failure. The tested range sits within ASHRAE TC 9.9's recommended envelope (~18 to 27°C for Class A1 data centers) and the model doesn't test beyond it.
...
Zero additional installed capacity. This is the architecture applied in the final model, and every metric elsewhere in this document reflects the corrected state.
...
The exact tipping point sits at **~69.5 kW**. 70 kW itself already crosses it, by a margin so thin (−0.5 kWth) that it is effectively the boundary, not a comfortable cutoff.
...
Cooling stays the binding constraint throughout the tested range. Electrical (118 kW) and UPS (144 kW) headroom remain positive even at 100 kW added, which is the practical point: **a site can run out of cooling while power capacity is still available.**
...
**Thermal grace period:** the degradation alarm fires at 03:22 and the room temperature alarm doesn't trigger until 04:05. That 43-minute window, driven by thermal inertia and redundant compensation, is the simulated intervention window an operations team has before the defined room-temperature threshold is reached.

## Model validation

Every claim above rests on an explicit check, not on an assumption that the model "should" be consistent.
...
- Support real-time incident detection. The 15-minute interval is fit for capacity and trend analysis, not for the faster sampling (typically 1 to 5 minutes) a real monitoring system would use to catch a developing fault as it happens

Every figure here is either a direct simulation output or an explicitly labelled assumption. Where a result depends on one (COP sensitivity, floor area, headcount, power factor), that dependency is stated next to the number, not buried in a footnote.

**Note on equipment sizing.** Transformers are nameplate-rated in kVA, not kW. The 800 kW figure used throughout is the active power derived from a 1,000 kVA nameplate at a 0.8 power factor, following Schneider Electric White Paper 3, *Calculating Total Power Requirements for Data Centers*. That conversion is an input assumption rather than a stored quantity: apparent power is computed from the meter's own `PowerFactor` column where it matters (§7), but no kVA nameplate is held in the underlying data.

## Sample data & sources

Full 28-day tables aren't included here (5,000 to 8,000 rows each). `data/` contains a **2-day illustrative extract** (9 and 10 August: one normal day, one incident day) plus the complete 14-row alarm log. The full dataset and the Power BI model are available on request.
